...
- 既に存在する実装要件ドキュメント（specification.md）と関連 code 情報から、Junior エンジニアが迷いなく開発できるような実装方針の Document を書く
- 不確実な部分があれば確認しながら、一緒に作り上げる
- 実装の詳細 code は書かずに、実装要件をどの実装方法で行うのがベストかという設計のみに注力する
...
- 既に存在する実装要件ドキュメントと関連 code 情報を参照する
...
- **設計方針書**: 作業中のブランチ名に対応するディレクトリに design-doc.md を配置します
...
- **要件定義書（更新がある場合）**: 同じディレクトリの specification.md を更新します
